...
function SequenceError:calculateCER(targetTranscript, predictTranscript)
    ...
    local targetCharacters, predictedCharacters = {}, {}
    local localCharacters = {}
    local function addToTable(s)
        table.insert(localCharacters, s)
    end
    utils.chsFilter(targetTranscript, addToTable, {'*'})
    targetCharacters = localCharacters
    localCharacters = {}
    utils.chsFilter(predictTranscript, addToTable, {'*'})
    predictedCharacters = localCharacters
    return self:sequenceErrorRate(targetCharacters, predictedCharacters)
end
