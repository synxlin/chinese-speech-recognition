require 'rnn'
require 'nngraph'
function makeDataParallel(model, nGPU, cudnnFastest)
    if nGPU > 0 then
        cudnn.fastest = cudnnFastest or true
...
function loadDataParallel(modelPath, nGPU, cudnnFastest)
    if nGPU > 1 then
        require 'cudnn'
        require 'networks.BatchBRNN'
    end
    ...
    if torch.type(model) == 'nn.DataParallelTable' then
        return makeDataParallel(model:get(1):float(), nGPU, cudnnFastest)
    elseif torch.type(model) == 'nn.Sequential' then
        for i, module in ipairs(model.modules) do
            if torch.type(module) == 'nn.DataParallelTable' then
                model.modules[i] = makeDataParallel(module:get(1):float(), nGPU, cudnnFastest)
            end
        end
        return model
    elseif torch.type(model) == 'nn.gModule' then
        model = makeDataParallel(model, nGPU, cudnnFastest)
        return model
    else
