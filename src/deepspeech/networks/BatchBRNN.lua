...
function BatchBRNN:__init(inputDim, outputDim, rnnType)
    parent.__init(self)
    ...
    rnn.numDirections = 2
    if rnnType == 'lstmBN' then
        rnn.mode = 'CUDNN_LSTM'
    elseif rnnType == 'gruBN' then
        rnn.mode = 'CUDNN_GRU'
    elseif rnnType == 'rnnReluBN' then
    ...
    self:add(rnn)
    if rnnType == 'rnnReluBN' then
        self:add(nn.Clamp(0, 20))
    end
